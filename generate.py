...
stop_words = {'it', "you'd", 'are', 'the', 'up', 'those', 'themselves', 'hers', 'on', 'when', 'is', 'ma', 'you', 'as', 'over', 'at', 'am', 'while', 'won',
  ...
  'were', "won't", 'of', 'most', 'nor', 'just', 'yourself', 'how', "aren't", 'under', "wouldn't", "wasn't", 'll', 'myself', 'ain', 'then', 'has', 'aren', "shan't", 'your', 'few', "you're", 'm', 'if', 'same', 'itself', 'should'}
...
def feature_create(ques1, ques2):
    ...
    # Handle zero or small denominator in rcosine calculation
    if rcosine_denominator == 0:
        rcosine = 0
    else:
    ...
    # Handle zero or small denominator for other features
    ...
    x['len_char_q1'] = len(str(ques1).replace(' ', ''))
    x['len_char_q2'] = len(str(ques2).replace(' ', ''))
    x['diff_len_char'] = x['len_char_q1'] - x['len_char_q2']
    x['len_word_q1'] = nonZero(len(str(ques1).split()), -1)
    x['len_word_q2'] = nonZero(len(str(ques2).split()), -1)
    x['diff_len_word'] = x['len_word_q1'] - x['len_word_q2']
    x['avg_word_len1'] = x['len_char_q1'] / x['len_word_q1']
    x['avg_word_len2'] = x['len_char_q2'] / x['len_word_q2']
    x['diff_avg_word'] = x['avg_word_len1'] - x['avg_word_len2']
    x['exactly_same'] = (ques1 == ques2)
    x['duplicated'] = (ques1 == ques2)
